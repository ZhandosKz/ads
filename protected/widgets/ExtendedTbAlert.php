 ...
 */
Yii::import('ext.bootstrap.widgets.TbAlert');
/**
 * Bootstrap alert widget.
 *
 * @see http://twitter.github.com/bootstrap/javascript.html#alerts
 */
class ExtendedTbAlert extends TbAlert
{
	// Alert types.
	...
	public function run()
	{
		...
		foreach ($this->alerts as $type => $alert) {
			...
			if (Yii::app()->getComponent($this->userComponentId)->hasFlash($type)) {
				...
				$flash = Yii::app()->getComponent($this->userComponentId)->getFlash($type);

				if (is_array($flash))
				{
					foreach ($flash as $flashMessage)
					{
						$this->_renderAlert($alert, $flashMessage);
					}
				}
				else
				{
					$this->_renderAlert($alert, $flash);
				}
			}
		}
	...
	private function _renderAlert(Array $alertOptions, $message)
	{
		echo CHtml::openTag('div', $alertOptions['htmlOptions']);
		...
		if (!isset($alertOptions['closeText'])) {
			$alertOptions['closeText'] = (isset($this->closeText) && $this->closeText !== false)
				? $this->closeText
				: false;
		}

		// If `closeText` which is in effect now is `false` then do not show button.
		if ($alertOptions['closeText'] !== false) {
			echo '<a href="#" class="close" data-dismiss="alert">' . $alertOptions['closeText'] . '</a>';
		}
